...
/**
 * Helper to add syntax highlighting to code blocks
 * Works with highlight.js which we're keeping from rehype-highlight
 */
export async function addSyntaxHighlighting(html: string): Promise<string> {
	...
	// Register common languages (can expand as needed)
	const languages = ["javascript", "typescript", "python", "java", "cpp", "rust", "go", "bash", "json", "xml"]
	...
	codeBlocks.forEach((block) => {
		...
		if (lang && lang !== "mermaid") {
			try {
				const result = hljs.default.highlight(block.textContent || "", { language: lang })
				block.innerHTML = result.value
			} catch {
				// Highlighting failed, keep original
			}
		}
	})
